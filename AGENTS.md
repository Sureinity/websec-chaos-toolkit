...
- Current behavior:
  - `toolkit audit` derives an ad hoc target from a single URL, executes a
    ...
    same-origin route set for Nuclei; preserves completed-tool findings and a
    secret-safe execution summary even when a later core scanner fails;
    omitted intensity behaves like `safe` and explicit `balanced` and `deep`
    modes increase bounded route and scanner budgets without changing the
